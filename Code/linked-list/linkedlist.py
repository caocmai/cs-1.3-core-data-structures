...
class LinkedList(object):
    ...
    def get_at_index(self, index):
        ...
        Best case running time: O(1) When index is at the head of linked list
        Worst case running time: O(n) When the index is at the tail of the linked list"""
    ...
    def insert_at_index(self, index, item):
        ...
        raise ValueError if the given index is out of range of the list size.
        Best case running time: O(1) When index to insert is at the head of linked list
        Worst case running time: O(n) When the index to insert is at the tail of the linked list"""
        # Check if the given index is out of range and if so raise an error
        if not (0 <= index <= self.size):
            raise ValueError('List index out of range: {}'.format(index))
        ...
        # Insert at tail
            ...
            target_node = self.head # Start

            i = 0
            while i < index:
                target_node = target_node.next # Target
                i += 1

            new_node.next = target_node
            target_node = new_node # Set the target node now to be the new node
    ...
    def prepend(self, item):
        ...
        Run time(Best and Worst): O(1) because like tail, the head can be accessed in one step"""
        # Create a new node to hold the given item
        new_node = Node(item)
        # Check if this linked list is empty
        if self.is_empty():
